...
static void
build_multitriggers_aux(unsigned count,
                        unsigned total,
                        Tstack_DAG elements,
                        Tstack_DAG *Pcandidate,
...
add new elements to candidate */
        ...
        {
          assert(count + max < total);
          build_multitriggers_aux(
              count + max, total, elements, Pcandidate, Presult);
        }
          ...
          for (j = i + 1; j < stack_size(candidates); ++j)
            {
              ...
              if (unitrigger_match(
                      DAG_args(DAG), DAG_arity(DAG) - 1u, DAG1, DAG2))
                break;
            }
