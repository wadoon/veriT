  ...
  for (i = 0; i < stack_size(option_table); i++)
    {
      ...
      if (option.key == key)
        switch (option.type)
          {
            case OPTION_BOOL: *(option.val.abool) = true; return 0;
            case OPTION_INT:
              if (!arg)
            ...
            case OPTION_STR:
              ...
              *(option->val.astr) = strmake(arg);
              return 0;
            default: my_error("options: internal error\n");
          }
    }
...
void
options_new_int_set_default(char short_name,
                            char *long_name,
                            char *doc,
                            char *arg,
                            int *aint,
        ...
        switch (option.type)
          {
            case OPTION_BOOL:
              ...
              break;
            case OPTION_INT: *(option.val.aint) = atoi(getenv(str)); break;
            case OPTION_STR:
              if (*(option->val.astr))
                free(*(option->val.astr));
  ...
  for (i = 0; i < stack_size(option_table); i++)
    {
      ...
      if (Poption->key == key)
        switch (Poption->type)
          {
            case OPTION_BOOL: *(Poption->val.abool) = true; return 1;
            case OPTION_INT:
              if (!arg)
                my_error("No argument given for option %s\n", Poption->key);
...
+ 4 to avoid spurious warning while memory checking optimized version */
      char *str =
          malloc(strlen(options_env_prefix) + strlen(Poption->name) + 4);
      strcpy(str, options_env_prefix);
        ...
        switch (Poption->type)
          {
            case OPTION_BOOL:
              ...
              break;
            case OPTION_INT: *(Poption->val.aint) = atoi(getenv(str)); break;
            case OPTION_STR:
              if (*(Poption->val.astr))
                free(*(Poption->val.astr));
          ...
          switch (opt[1])
            {
              ...
              default:
                if (opt[2] != '\0')
                  {
                    fprintf(
                        stderr, "options: error in command line (%s)\n", opt);
                    options_usage(stderr);
                    exit(-1);
                  }
      ...
      switch (Poption->type)
        {
          case OPTION_BOOL:
            ...
            break;
          case OPTION_INT: fprintf(file, "%d\n", *(Poption->val.aint)); break;
          case OPTION_STR: fprintf(file, "%s\n", *(Poption->val.astr)); break;
          default: my_error("options: internal error\n");
        }
    }
}
...
void
options_setup(Tstack_Pchar arg_table,
              char *name,
              char *version,
              char *bug_address,
              char *doc_args,
              ...
              char *env_prefix)
{
  ...
  options_program_doc_foot =
      strapp(options_program_doc_foot,
             &sz,
  ...
  options_program_doc_foot = strapp(options_program_doc_foot, &sz, doc_foot);
#ifdef WITH_ARGP
  sz = 0;
  ...
  options_new(
      '\0', "usage", "prints help message and exits", &options_print_usage);
#endif
}
