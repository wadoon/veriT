  ...
  for (i = stack_size(constraints); i > 0; --i)
    {
      ...
      /* stack_get(constraints, i - 1) belongs to the component components[i -
       * 1] */
    }
  /* Collect components */
...
variable */
  ...
  assert((sig_D0 && sig_D1) || (fresh_D0 && fresh_D1) || (fresh_D0 && sig_D1) ||
         (fresh_D1 && sig_D0));
  /* TODO: simplify this somehow */
  /* DAG is a var, leave to be handled later */
  if (!constr->pol && !ccfv_all_CI && !ground(constr->D0) &&
      !DAG_arity(constr->D0))
    {
  ...
  if (!constr->pol && !ccfv_all_CI && !ground(constr->D1) &&
      !DAG_arity(constr->D1))
    {
      if (fresh_D0)
        {
  ...
  /* If sigs, just check with ground model, otherwise try E-matching
   * args/assigns (!pol) */
  if (sig_D0 && sig_D1)
    {
      constr->D0 = sig_D0;
...
/* x = y or x = t or ... */
static void
classify_var_gr_or_var(Tunifier solution,
                       TDAG NGDAG,
                       TDAG UDAG,
                       Tconstr *constr)
{
...
/* TODO: to handle in a more directed way than to simply call
   classify_constraint CCFV_ASSIGN, CCFV_PRED, CCFV_EMATCH, CCFV_EUNI_VAR,
   CCFV_EUNI_FAPP,
   CCFV_ASSIGN_FAPP
*/
void
...
check_ground_apps(Tstack_constr constraints)
{
   ...
   */
  /*             { */
  /*               /\* my_DAG_error("This guy should have been killed
   * before\n"); *\/ */
  /*               return false; */
  /*             } */
  /*           for (j = 0; j < stack_size(index.signatures[pol]); ++j) */
  /*             if (congruent(arg_DAG, */
  /*                           DAG_arg(stack_get(index.signatures[pol], j),
   * pos))) */
  /*               break; */
