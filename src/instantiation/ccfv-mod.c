...
would have this called on twice when x is grounded */
  if (ground_mod(DAG))
    return;
  for (i = 0; i < DAG_arity(DAG); ++i)
    if (!ground(DAG_arg(DAG, i)) && ground_mod(DAG_arg(DAG, i)) &&
        !check_arg_ground(DAG, i))
      set_arg_ground(DAG, i);
...
print_DAG_modulo(TDAG DAG)
{
  ...
  my_DAG_message("{%d}%D -> %s/%D\n",
                 DAG,
                 DAG,
                 byte_to_binary(DAGs_modulo[DAG]->ground_args,
                                DAG_arity(DAG) ? DAG_arity(DAG) : 1),
                 DAGs_modulo[DAG]->term);
}
