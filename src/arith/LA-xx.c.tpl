...
#include "arith/LA-xx.h"
#include "arith/eq-store.h"
#include "arith/numbers-xx.h"
#include "arith/simplex-xx.h"
...
static TLArational_xx* prior_coefficient_var = 0;
...
LA_DAG_var(TDAG DAG)
{
	...
	if (!DAG_var_table->data[DAG])
		DAG_var_table->data[DAG] = simplex_xx_var_new(
...
{
	unsigned i;
	for (i = 0; i < var_assoc->n; i++) LAsigned_xx_clear(var_assoc->coefs[i]);
	free(var_assoc->vars);
...
LA_constraint_push2(TDAG DAG)
{
	...
	if (stack_size(monoms) == 1) {
		...
		if (LArational_xx_is_neg(monoms->data[0].coef)) upper = !upper;
		var = monoms->data[0].var;
		if (proof_on) {
			LArational_xx_init(prior_coeff);
			LArational_xx_set_one(prior_coeff);
			LArational_xx_div(prior_coeff, monoms->data[0].coef);
		}
		goto allocate;
	}
	...
	if (proof_on) {
		LArational_xx_init(prior_coeff);
		LArational_xx_set_si(prior_coeff, tmp);
	}
	...
	if (proof_on) {
		LArational_xx_div_s(prior_coeff, tmp);
	}
...
allocate:
	...
	if (proof_on) {
		if (prior_var_alloc <= var_max) {
			MY_REALLOC(prior_coefficient_var, 2 * var_max * sizeof(TLArational_xx));
			for (unsigned i = prior_var_alloc; i < 2 * var_max; ++i) {
				memset(prior_coefficient_var[i], 0, sizeof(TLArational_xx));
			}
			prior_var_alloc = 2 * var_max;
		}
		LArational_xx_init(prior_coefficient_var[atom]);
		if (LArational_xx_is_neg(prior_coeff))
			LArational_xx_neg(prior_coeff);
		LArational_xx_set(prior_coefficient_var[atom], prior_coeff);
		LArational_xx_clear(prior_coeff);
	}
	...
	if (simplex_xx_var_integer(var)) {
		...
		*/
		if (LArational_xx_is_int(indep_term)) /* implements the first table */
		{
			...
			LAdelta_xx_set(&bounds->data[atom].delta2, &bounds->data[atom].delta);
			if (upper)
				if (strict)
		...
		LAdelta_xx_set_delta(&bounds->data[atom].delta, upper ? -strict : strict);
		LAdelta_xx_set_delta(
...
static inline void
LA_xx_notify_monom(
	const TLArational_xx rat, const Tsimplex_var var, const Tpol_b pol)
{
	stack_inc(monoms);
	LArational_xx_init(stack_top(monoms).coef);
	LArational_xx_set(stack_top(monoms).coef, rat);
	if (pol == POL_NEG) LArational_xx_neg(stack_top(monoms).coef);
...
static void LA_xx_notify_term(const TDAG, const Tpol_b, const TLArational_xx);
...
static inline bool
LA_xx_notify_term_top(
	const TDAG DAG, const Tpol_b pol, const TLArational_xx fact)
{
	TLArational_xx rat;
	unsigned i;
#if DEBUG_LA >= 2
...
/**
   ...
   \see LA_xx_notify_term, LA_xx_notify_term_top */
static void
LA_xx_notify_term(const TDAG DAG, const Tpol_b pol, const TLArational_xx fact)
{
	unsigned i;
	...
	if (LA_xx_notify_term_top(DAG, pol, fact)) return;
	LA_xx_notify_monom(fact, LA_DAG_var(DAG), pol);
...
LA_xx_unate_propagation(void)
{
	...
	while (i + 1 < n) {
		...
		while (bounds->data[aj].var == v && bounds->data[aj].upper) {
			...
			if (LAdelta_xx_eq(&bounds->data[ai].delta, &bounds->data[aj].delta))
				/* (i : v <= b_i), (j : v <= b_j), b_i = b_j
               |= (j) => (i) -- lemma */
		...
		while (bounds->data[ai].upper && !bounds->data[aj].upper) {
			assert(bounds->data[ai].var == v);
			assert(bounds->data[aj].var == v);
			if (LAdelta_xx_less(&bounds->data[ai].delta, &bounds->data[aj].delta)) {
				/* (i : v <= b_i), (j : v >= b_j), b_i < b_j
                 |= (i) => !(j) -- lemma */
				if (
...
LA_xx_assert_eq(TDAG DAG0, TDAG DAG1)
{
	...
	if (!svar) {
		...
		if (simplex_xx_solve() != SAT) return UNSAT;
		LArational_xx_init(rat);
		LArational_xx_set_one(rat);
	...
	if (
		simplex_xx_assert_lower_bound(svar, store.assert_eq.delta, 0) == UNSAT ||
...
LA_xx_conflict_proof(void)
{
	...
	assert(!LA_xx_intg_conflict);
	Tstack_LAsigned_xx conflict_coeffs;
	stack_INIT(conflict_coeffs);
	simplex_xx_conflict_proof(&conflict_lits, &conflict_eqs, &conflict_coeffs);
	stack_INIT(lits);
	...
	for (unsigned i = 0; i < stack_size(conflict_lits); ++i) {
		...
		TLArational_xx c;
		LArational_xx_init(c);
		if (!LAsigned_xx_is_zero(stack_get(conflict_coeffs, coef_idx)))
			LArational_xx_set_si(c, stack_get(conflict_coeffs, coef_idx));
		else
			LArational_xx_set_one(c);

		assert(!LArational_xx_is_zero(prior_coefficient_var[SAT_lit_var(lit)]));
		LArational_xx_mult(c, prior_coefficient_var[SAT_lit_var(lit)]);
	...
	/* I can't use stack_apply, because LAsigned_xx_clear is a macro */
	for (unsigned i = 0; i < stack_size(conflict_coeffs); ++i) {
		LAsigned_xx_clear(stack_get(conflict_coeffs, i));
	}
...
LA_xx_model_eq(void)
{
	...
	for (i = 0; i < n - 1;) {
		...
		do {
			...
			if (!simplex_xx_var_eq(sv_i, sv_j)) break;
	...
	unsigned d, unsigned m, Tstack_lit* Plits, Tstack_simplex_var* Peqs)
{
	...
	for (it = simplex_integer_var_begin(); it != simplex_integer_var_end();
		...
		if (!LAdelta_xx_is_int(Pdelta)) break;
	}
...
LA_xx_init(void)
{
	...
	if (var_max < 64)
		...
		prior_var_alloc = 2 * var_max;
	MY_MALLOC(prior_coefficient_var, prior_var_alloc * sizeof(TLArational_xx));
	memset(prior_coefficient_var, 0, prior_var_alloc * sizeof(TLArational_xx));
}
...
LA_xx_done(void)
{
	size_t i;
	stats_unsigned(
		"lemmas/bounds_xx", "Bound lemmas generated by arith (XX version)", "%9u",
		LA_lemma_n);
	...
	for (unsigned i = 0; i < prior_var_alloc; ++i) {
		if (prior_coefficient_var[i] != 0)
			LArational_xx_clear(prior_coefficient_var[i]);
	}
